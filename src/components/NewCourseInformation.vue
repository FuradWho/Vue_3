<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>可选修课程</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
          ...
          <template slot-scope="scope">
            <el-tooltip effect="dark"
              ...
              <el-button type="primary" icon="el-icon-edit" size="mini"
                         @click="chooseCourse(scope.row.courseId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async getCourseList() {
      const {data: res} = await this.$http.get('/getALLCourseList', {
    ...
    async chooseCourse(id){
      const confirmResult  = await this.$confirm('此操作将选修该课程, 是否继续?', '提示', {
      ...
      if (confirmResult !== 'confirm') {
        return this.$message.info('已取消选修')
      }
      console.log(id)

      const {data : res} =await this.$http.post('/chooseCourse',id)

      if(res.code !== 0){
        return this.$message.error('选修课程失败！')
      }
